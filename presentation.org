...
- To /satisfy/ is to find a world (of values) where all given patterns/rules can be true.
- To /unify/ is to see if a rule or pattern can be made equal to another.
- We find answers via unification.
